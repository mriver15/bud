...
public struct MCPAppWebView: NSViewRepresentable {
    ...
    public func makeCoordinator() -> MCPAppCoordinator {
        MCPAppCoordinator(resource: resource, attachment: attachment)
    }
...
/// view at a reading height, a thin border when the server asked for one, and a
/// caption that says what it is and where it came from.
///
...
public struct MCPAppShell: View {
    ...
    public var body: some View {
        VStack(alignment: .leading, spacing: Bud.Space.xs) {
            MCPAppWebView(resource: resource, attachment: attachment)
                .frame(height: 520)
